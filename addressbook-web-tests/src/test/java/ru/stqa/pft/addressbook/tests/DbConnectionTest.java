...
public class DbConnectionTest {
  ...
  public void testContactDbConnection(){
    ...
    try {
      conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/addressbook?user=root&password=");
      Statement st = conn.createStatement();
      ResultSet rs = st.executeQuery("select id,firstname,lastname,email,home,address from addressbook");
      ...
      while (rs.next()) {
        contacts.add(new ContactData().withId(rs.getInt("id")).withFirstname(rs.getString("firstname"))
                ...
                .withEmail(rs.getString("email")).withHomePhone(rs.getString("home")));
      }
      rs.close();
      st.close();
      conn.close();
